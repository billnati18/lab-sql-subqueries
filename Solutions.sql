...
JOIN
    ...
    FROM 
        film 
    WHERE title = 'Hunchback Impossible') AS film_filtered 
ON 
    film_filtered.film_id = inventory.film_id;
...
HAVING COUNT(film_actor.film_id) = (SELECT 
    ...
    FROM
        (SELECT 
            actor.actor_id, COUNT(film_actor.film_id) AS movies_count
        FROM
            actor
        JOIN film_actor ON actor.actor_id = film_actor.actor_id
